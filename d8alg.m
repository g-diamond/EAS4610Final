%Joshua Carmichael (2021). D8 Algorithm for Hydrological Models
%(https://www.mathworks.com/matlabcentral/fileexchange/21682-d8-algorithm-for-hydrological-models),
...
%corners are done
for i = 2:M-1 %sum over number of northward cells (rows)
    
    for j = 2:N-1 %sum over number of eastward cells (columns)
        
        % Assign values to variable input arguments.
        subPsi              = Psi(i-1:i+1,j-1:j+1);
        [temp,I]            = min(subPsi(:));
        [I,J]               = ind2sub([3,3],I);
        %the minus 2 is needed since the min is computed over a 3x3 array
        flowDir{i,j}        = [I+i-2,J+j-2];
...
%Get the cumulative sum matrix
for i = 1:M %sum over number of northward cells (rows)
    
    for j = 1:N %sum over number of eastward cells (columns)
        ...
        if( and(ind1==i,ind2==j) ),
            ...
            Accum(ind1,ind2) = Accum(i,j) + Accum(ind1,ind2);
